...
pub async fn request_payout(
    ...
    let payout_id = state.db.request_payout(user_id, amount, currency, wallet_address, network)
        .map_err(|e| (StatusCode::BAD_REQUEST, e.to_string()))?;

    let _ = state.db.create_receipt(user_id, "payout", payout_id, amount, currency, &format!("Payout to {}", wallet_address), "{}");
...
pub async fn get_my_payouts(
    ...
    let payouts = state.db.get_user_payouts(user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn get_pending_payouts(
    ...
    let payouts = state.db.get_pending_payouts()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn process_payout(
    ...
    state.db.process_payout(payout_id, user_id, tx_hash, approved)
        .map_err(|e| (StatusCode::BAD_REQUEST, e.to_string()))?;
