...
pub async fn send_gift(
    ...
    let _ = state.db.create_receipt(from_user, "gift", gift_record_id, 0, "YSH", &format!("Gift #{}", gift_id), "{}");

    let catalog = state.db.get_gift_catalog().unwrap_or_default();
    let rarity = catalog.iter().find(|g| g["id"].as_i64() == Some(gift_id))
...
pub async fn get_sent_gifts(
    ...
    let gifts = state.db.get_sent_gifts(user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn get_gift_stats(
    ...
    let stats = state.db.get_gift_stats(user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn get_nft_gifts(
    ...
    let nfts = state.db.get_nft_gifts(user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
